...
import {
  COMO_GARDA_BOOKING_TERMS,
  COMO_GARDA_EXCLUDED_LIST,
  COMO_GARDA_INCLUDED_LIST,
  COMO_GARDA_POST_TABLE_NOTICE,
} from '../data/comoGardaLegal'
import {
  COMO_GARDA_BOOKING_TERMS_EN,
  COMO_GARDA_EXCLUDED_LIST_EN,
  COMO_GARDA_INCLUDED_LIST_EN,
  COMO_GARDA_POST_TABLE_NOTICE_EN,
} from '../data/comoGardaLegal.en'
import {
  COMO_GARDA_PRICE_SUMMARY_LINE,
  COMO_GARDA_PROGRAM_DAYS,
  COMO_GARDA_TOUR_PDF_HREF,
} from '../data/comoGardaProgram'
import { COMO_GARDA_PRICE_SUMMARY_LINE_EN, COMO_GARDA_PROGRAM_DAYS_EN, COMO_GARDA_TOUR_PDF_HREF_EN } from '../data/comoGardaProgram.en'
...
const heroImage = '/images/resorts/valmalenco/3.webp'
...
const ItalyComoPage = () => {
  ...
  const pdfAbsoluteHref = useMemo(
    () => absolutePublicUrl(en ? COMO_GARDA_TOUR_PDF_HREF_EN : COMO_GARDA_TOUR_PDF_HREF),
    [en],
  )

  const priceSummaryLine = en ? COMO_GARDA_PRICE_SUMMARY_LINE_EN : COMO_GARDA_PRICE_SUMMARY_LINE
  const programDays = en ? COMO_GARDA_PROGRAM_DAYS_EN : COMO_GARDA_PROGRAM_DAYS
  const postTableNotice = en ? COMO_GARDA_POST_TABLE_NOTICE_EN : COMO_GARDA_POST_TABLE_NOTICE
  const includedList = en ? COMO_GARDA_INCLUDED_LIST_EN : COMO_GARDA_INCLUDED_LIST
  const excludedList = en ? COMO_GARDA_EXCLUDED_LIST_EN : COMO_GARDA_EXCLUDED_LIST
  const bookingTerms = en ? COMO_GARDA_BOOKING_TERMS_EN : COMO_GARDA_BOOKING_TERMS
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: t('italyTourPages.schemaItaly'), item: 'https://vacanzabianca.ru/italy/' },
          {
            '@type': 'ListItem',
            position: 3,
            name: t('italyTourPages.comoLake.breadcrumbCurrent'),
            item: CANONICAL,
          },
        ],
      },
  ...
  useEffect(() => {
    ...
    upsertMeta({
      title: t('italyTourPages.comoLake.metaTitle'),
      description: t('italyTourPages.comoLake.metaDescription'),
      canonical: CANONICAL,
  ...
  return (
    <div className="min-h-screen bg-bg-base">
      ...
      <BreadcrumbOverlay
        ...
        homeHref="/italy/"
        currentLabel={t('italyTourPages.comoLake.breadcrumbCurrent')}
      ...
      <main className="pb-12 pt-8 sm:pt-10 md:pt-12">
        <section className="mx-auto w-full max-w-[1200px] px-4 sm:px-6 md:px-8 lg:px-5">
          <div className="mb-10 flex flex-col items-center gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
            <div className="w-full flex-1 text-center sm:text-left">
              <h1 className="section-title !mb-4 text-center sm:!text-left">{t('italyTourPages.comoLake.h1')}</h1>
        ...
        <section className="mx-auto mt-16 w-full max-w-[1200px] px-4 sm:px-6 md:px-8 lg:px-5">
          <div className="rounded-2xl border border-border-soft bg-bg-card px-6 py-10 text-center md:px-10 md:py-14">
            <h2 className="section-title !mb-10">{t('italyTourPages.comoLake.ctaTitle')}</h2>
            <p className="mx-auto mb-8 max-w-[760px] text-sm leading-7 text-text-light">{t('italyTourPages.comoLake.ctaBody')}</p>
            <button
              type="button"
